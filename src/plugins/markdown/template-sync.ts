...
export async function templateSync(context: PluginContext, options: TemplateSyncOptions): Promise<true | null> {
  const { templateDir, templatePrefix = '', fieldMap = {} } = context.pluginConfig as MarkdownPluginConfig;
  if (!templateDir) {
