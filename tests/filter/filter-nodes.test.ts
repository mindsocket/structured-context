...
describe('filterNodes', () => {
  describe('WHERE clause matching', () => {
    it('filters by resolvedType', async () => {
      const result = nodes(await filterNodes("WHERE resolvedType='solution'", allGraph));
      expect(result).toHaveLength(2);
      expect(result.map((n) => n.title)).toContain('Solution 1');
      expect(result.map((n) => n.title)).toContain('Solution 2');
    });

    it('filters by a schemaData field (status)', async () => {
      const result = nodes(await filterNodes("WHERE status='active'", allGraph));
      expect(result.map((n) => n.title)).toEqual(['My Goal', 'Active Opportunity', 'Solution 1']);
    });

    it('filters by combined conditions', async () => {
      const result = nodes(await filterNodes("WHERE resolvedType='solution' and status='active'", allGraph));
      expect(result).toHaveLength(1);
      expect(result[0]!.title).toBe('Solution 1');
    });

    it('returns empty graph when nothing matches', async () => {
      const result = await filterNodes("WHERE resolvedType='nonexistent'", allGraph);
    ...
    it('supports bare JSONata without WHERE keyword', async () => {
      const result = nodes(await filterNodes("resolvedType='solution'", allGraph));
      expect(result).toHaveLength(2);
    });
  });
  ...
  describe('no filter predicate', () => {
    it('returns all nodes when no WHERE clause', async () => {
      const result = await filterNodes('SELECT ancestors(opportunity)', allGraph);
      // SELECT-only: returns all nodes (no WHERE filter)
      expect(result.nodes.size).toBe(allNodes.length);
    });
  });
  ...
  describe('SELECT clause expansion', () => {
    it('expands result with SELECT ancestors when present', async () => {
      ...
      const result = nodes(await filterNodes("SELECT ancestors(opportunity) WHERE resolvedType='solution'", allGraph));
    ...
    it('SELECT-only returns all nodes expanded', async () => {
      ...
      expect(result.nodes.size).toBeGreaterThanOrEqual(allNodes.length);
    });
  });

  describe('return type', () => {
    it('returns original SpaceNode objects, not augmented representations', async () => {
      const result = nodes(await filterNodes("WHERE resolvedType='solution'", allGraph));
    ...
    it('returns a SpaceGraph with correct classification', async () => {
      const result = await filterNodes("WHERE resolvedType='solution'", allGraph);
      // Solutions without their parents are orphans in the filtered graph
