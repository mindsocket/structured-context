...
describe('buildChildrenIndex', () => {
  it('builds empty index for nodes with no parents', () => {
    const root = makeNode('Root', 'goal');
    const index = buildChildrenIndex([root]);
    expect(index.get('Root')).toEqual([]);
  });

  it('maps parent titles to their children', () => {
    const root = makeNode('Root', 'goal');
    const child = makeNode('Child', 'opportunity', [makeParentRef('Root')]);
    const index = buildChildrenIndex([root, child]);
    expect(index.get('Root')).toEqual([child]);
    expect(index.get('Child')).toEqual([]);
  });
  ...
  it('supports multiple children per parent', () => {
    ...
    const index = buildChildrenIndex([root, child1, child2]);
...
describe('augmentNode', () => {
  describe('ancestors', () => {
    it('returns empty ancestors for a root node', () => {
      ...
      const childrenIndex = buildChildrenIndex([root]);
      const result = augmentNode(root, nodeIndex, childrenIndex);
    ...
    it('includes direct parent as ancestor with edge metadata', () => {
      ...
      const childrenIndex = buildChildrenIndex([root, child]);
      const result = augmentNode(child, nodeIndex, childrenIndex);
    ...
    it('handles cycle detection for selfRef nodes', () => {
      ...
      const result = augmentNode(solutionB, nodeIndex, childrenIndex);
  ...
  describe('descendants', () => {
    it('returns empty descendants for a leaf node', () => {
      ...
      const childrenIndex = buildChildrenIndex([leaf]);
      const result = augmentNode(leaf, nodeIndex, childrenIndex);
  ...
  describe('flat node fields', () => {
    it('includes schemaData fields at the top level', () => {
      ...
      const childrenIndex = buildChildrenIndex([node]);
